    ...
    <Box marginTop={"8"}>
      ...
      <TableContainer display={"block"}>
        <Table variant={"striped"} colorScheme={"gray"} size={"sm"}>
          ...
          <Tbody>
            {filtered &&
              filtered.map((transaction: any, index: number) => {
                return (
                  <>
                    <Tr key={index}>
                      <Td>swap</Td>
                      ...
                      <Td>
                        {Math.abs(transaction.swaps[0].amount0).toFixed(2)}{" "}
                        {transaction.swaps[0].token0.symbol}
                      </Td>
                      <Td>
                        {Math.abs(transaction.swaps[0].amount1).toFixed(2)}{" "}
                        {transaction.swaps[0].token1.symbol}
                      </Td>
                      <Td>
                        <Link
                          href={`https://etherscan.io/address/${transaction.swaps[0].sender}`}
                        >
